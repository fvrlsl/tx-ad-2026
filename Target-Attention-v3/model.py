...
# ═══════════════════════════════════════════════════════════════════════════════
# Tier 1: Target Attention + NS Feature Cross Layer
# ═══════════════════════════════════════════════════════════════════════════════


class NSFeatureCrossLayer(nn.Module):
    """NS tokens self-attention for explicit feature interaction (InterFormer style)."""
    ...
    def forward(self, ns_tokens: torch.Tensor) -> torch.Tensor:
        """Applies self-attention feature crossing among NS tokens.

        Args:
            ns_tokens: (B, M, D) non-sequence tokens.
        Returns:
            Crossed NS tokens of shape (B, M, D).
        """
        residual = ns_tokens
        x = self.norm1(ns_tokens)
...
class TargetAttention(nn.Module):
    """DIN-style target-aware attention module.

    Uses target item representation as query to attend over each sequence
    domain, extracting interest tokens relevant to the current candidate item.
    """

    def __init__(self, d_model: int, num_heads: int, num_sequences: int,
                 dropout: float = 0.0) -> None:
        super().__init__()
        self.num_sequences = num_sequences
        self.target_cross_attns = nn.ModuleList([
            CrossAttention(d_model=d_model, num_heads=num_heads,
                           dropout=dropout, ln_mode='pre')
            for _ in range(num_sequences)
        ])

    def forward(self, target_repr: torch.Tensor,
                seq_tokens_list: list, seq_masks_list: list) -> list:
        """Extracts target-aware interest from each sequence domain.

        Args:
            target_repr: (B, 1, D) target item representation.
            seq_tokens_list: List of (B, L_i, D) sequence token tensors.
            seq_masks_list: List of (B, L_i) padding masks (True=pad).
        Returns:
            List of (B, 1, D) interest tokens, one per sequence domain.
        """
        interest_tokens = []
        for i, cross_attn in enumerate(self.target_cross_attns):
            interest = cross_attn(
                query=target_repr,
                key_value=seq_tokens_list[i],
                key_padding_mask=seq_masks_list[i],
            )
            interest_tokens.append(interest)
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        # +1 for seq_pooled, +1 for interest_token (if target attention)
        extra_slots = 2 if use_target_attention else 1
        global_info_dim = (num_ns + extra_slots) * d_model
    ...
    def forward(
        ...
        interest_tokens: Optional[list] = None,
    ) -> list:
        ...
        Args:
            ...
            seq_padding_masks: List of (B, L_i) masks, length S. True
                indicates padding.
            interest_tokens: Optional list of (B, 1, D) target-aware interest
                tokens, one per sequence domain.
        ...
        for i in range(self.num_sequences):
            ...
            # GlobalInfo_i = Concat(NS_flat, seq_pooled_i [, interest_i])
            if self.use_target_attention and interest_tokens is not None:
                interest_i = interest_tokens[i].squeeze(1)  # (B, D)
                global_info = torch.cat([ns_flat, seq_pooled, interest_i], dim=-1)
            else:
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # ================== Tier 1: NS Feature Cross Layer ==================
        self.ns_cross_layer = NSFeatureCrossLayer(
            d_model=d_model,
            num_heads=num_heads,
            hidden_mult=hidden_mult,
        ...
        # ================== HyFormer Components ==================
        # MultiSeqQueryGenerator (enhanced with target interest)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1b. NS Feature Cross Layer (Tier 1): explicit feature interaction
        ns_tokens = self.ns_cross_layer(ns_tokens)  # (B, num_ns, D)
        ...
        # 2b. Target Attention (Tier 1, DIN): extract target-aware interest
        # Use item NS tokens mean as target representation
        target_repr = item_ns.mean(dim=1, keepdim=True)  # (B, 1, D)
        if self.has_item_dense:
            target_repr = target_repr + item_dense_tok  # fuse item dense info
        interest_tokens = self.target_attention(target_repr, seq_tokens_list, seq_masks_list)

        # 3. Generate Q tokens with target interest injection
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # NS Feature Cross
        ns_tokens = self.ns_cross_layer(ns_tokens)

        # Target Attention
        target_repr = item_ns.mean(dim=1, keepdim=True)
        if self.has_item_dense:
            target_repr = target_repr + item_dense_tok
        interest_tokens = self.target_attention(target_repr, seq_tokens_list, seq_masks_list)
